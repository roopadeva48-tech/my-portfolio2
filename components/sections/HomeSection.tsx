...
// Social Icons (kept for context, but not directly modified here)
const LinkedInIcon = () => (
...
// Tech Icons (with their respective hover colors)
const techIconsData = [
...
const AnimatedTechStack: React.FC = () => {
    ...
    const [collectedIcons, setCollectedIcons] = useState<number[]>([]);
    const [isAnimating, setIsAnimating] = useState(false);
    const techStackRef = useRef<HTMLDivElement>(null);
    const containerRef = useRef<HTMLDivElement>(null); // New ref for the main container

    useEffect(() => {
        if (!techStackRef.current || isAnimating) return;
        ...
        const animateCollection = async () => {
            ...
            const iconElements = techStackRef.current.querySelectorAll('.tech-icon-wrapper');
            if (iconElements.length === 0) return;

            // 1. Fly-in from right
            setAstronautStyle(prev => ({ 
                ...prev, 
                right: '100px', // Stop before the icons start
                transition: 'all 2s ease-in-out' 
            }));
            await new Promise(resolve => setTimeout(resolve, 2000)); 

            // 2. Collect icons sequentially
            for (let i = 0; i < techIconsData.length; i++) {
                const iconElement = iconElements[i] as HTMLElement;
                const rect = iconElement.getBoundingClientRect();
                const containerRect = techStackRef.current!.getBoundingClientRect();

                // Calculate center of the icon relative to the techStackRef container
                const targetLeft = rect.left - containerRect.left + (rect.width / 2) - 32; // -32 for half astronaut width (16x16)
                
                // Animate astronaut to the icon's position
                setAstronautStyle(prev => ({
                    ...prev,
                    left: `${targetLeft}px`,
                ...
                await new Promise(resolve => setTimeout(resolve, 1000)); // Time to reach icon

                // "Collect" the icon
                setCollectedIcons(prev => [...prev, i]);
    ...
    return (
        <div className="relative overflow-hidden" ref={containerRef}>
            {/* The absolute positioning of the astronaut requires the parent to be relative */}
            <AstronautSVG
                className="w-16 h-16 absolute z-20"
                style={astronautStyle}
            />

            <div className="flex flex-wrap gap-4 justify-center md:justify-start" ref={techStackRef}>
                {techIconsData.map((tech, index) => {
                    const IconComponent = tech.Icon;
                    const isCollected = collectedIcons.includes(index);

                    return (
                        <div 
                            key={tech.name} 
                            // Fade out and shrink effect on collection
                            className={`tech-icon-wrapper group relative transition-all duration-500 ${isCollected ? 'opacity-0 scale-75' : 'opacity-100 scale-100'}`}
                        >
                            <div 
                                className="text-gray-400 transition-colors duration-300"
                                style={{
                                    // Use CSS variables for hover color
