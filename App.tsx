...
const App: React.FC = () => {
  ...
  // If the page is opened with a hash (e.g. #contact) or the hash changes,
  // switch the active section so anchor links work even though sections
  // are mounted conditionally by `activeSection`.
  React.useEffect(() => {
    const mapHashToSection = (hash: string) => {
      switch ((hash || '').toLowerCase()) {
        ...
        case '#certificates':
        case '#certificate':
          return SectionType.CERTIFICATE;
        case '#about':
  ...
  const navItems = [
    ...
    { id: SectionType.SKILLS, label: 'Skills', icon: Icons.Skill },
