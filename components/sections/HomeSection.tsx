...
// --- NEW: Simplified Tech Stack for Scroll Reveal ---
const ScrollRevealTechStack: React.FC = () => {
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            ([entry]) => {
                // If the element is visible (intersectionRatio > 0) and we haven't revealed it yet
                if (entry.isIntersecting && !isRevealed) {
                    setIsRevealed(true);
                    // Optional: Unobserve after the reveal if it's a one-time animation
    ...
    // Small icons (w-8 h-8) by default, transition to large (w-12 h-12) on reveal.
    const iconBaseClasses = "transition-all duration-1000 ease-in-out transform";

    return (
        // The container needs a height for the Intersection Observer to work on scroll
        <div className="min-h-[100px] flex items-center" ref={containerRef}>
            <div className="flex flex-wrap gap-4 md:gap-8 justify-center md:justify-start w-full">
                {techIconsData.map((tech) => {
                    const IconComponent = tech.Icon;
                    
                    const iconSizeClass = isRevealed ? 'w-12 h-12 opacity-100 scale-100' : 'w-8 h-8 opacity-50 scale-75';

                    return (
                        <div 
                            key={tech.name} 
                            className={group relative ${iconBaseClasses} ${iconSizeClass}} // Apply size and transition
                        >
                            <div 
                            ...
                            <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-800 text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                                {tech.name}
                            </span>
                        </div>
                    );
...
// --- Home Section (MODIFIED: Replaced AnimatedTechStack with ScrollRevealTechStack) ---
const HomeSection: React.FC = () => {
    ...
    return (
        // Added min-h-screen to ensure the page is scrollable, allowing the observer to work
        <div className="w-full min-h-screen flex flex-col md:flex-row items-center justify-center p-6 md:p-12 gap-10">
            ...
            <div className="w-full md:w-1/2 space-y-6 animate-fade-in-up text-center md:text-left z-10">
                ...
                <div className="space-y-4">
                    ...
                    {/* The NEW component for scroll reveal */}
